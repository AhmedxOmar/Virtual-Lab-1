...
useEffect(() => {
    ...
    const fetchQuestions = async () => {
        try {
            // Fetch quiz questions from your backend API
            const response = await fetch(`http://localhost:8080/api/quizzes/${chapterId}`);
            if (!response.ok) throw new Error('Failed to fetch questions from database');

            const data = await response.json();

            // Shuffle and pick first 10 if needed
